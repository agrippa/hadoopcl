...
abstract class TaskRunner extends Thread {
  ...
  public TaskRunner(TaskTracker.TaskInProgress tip, TaskTracker tracker, 
                    JobConf conf, TaskTracker.RunningJob rjob, String type
                    ) throws IOException {
    this.type = type;
    this.tip = tip;
    this.t = tip.getTask();
  ...
  public final void run() {
    ...
    try {
      ...
      Vector<String> vargs = getVMArgs(taskid, workDir, classPaths, logSize,
          tip.getAssignedDevice(), tip.getAssignedDeviceSlot());
      ...
      launchJvmAndWait(setupCmds, vargs, stdout, stderr, logSize, workDir, type);
      ...
      try {
        tracker.reportDiagnosticInfoInternal(t.getTaskID(), "Throwable.toString: "+throwable.toString()+"\nThrowable.getMessage: "+throwable.getMessage()+"\nbaos: "+baos.toString());
      } catch (IOException e) {
  ...
  void launchJvmAndWait(List <String> setup, Vector<String> vargs, File stdout,
      File stderr, long logSize, File workDir, String type)
      throws InterruptedException, IOException {
    jvmManager.launchJvm(this, jvmManager.constructJvmEnv(setup, vargs, stdout,
        stderr, logSize, workDir, conf, type));
    synchronized (lock) {
      while (!done) {
        lock.wait();
      }
  ...
  private Vector<String> getVMArgs(TaskAttemptID taskid, File workDir,
    ...
    vargs.add("-Dopencl.device_slot=" + deviceSlot);
