...
public final class IntFsvecHadoopCLOutputReducerBuffer extends HadoopCLOutputReducerBuffer implements KVCollection<IntWritable, FSparseVectorWritable> {
    ...
    public IntFsvecHadoopCLOutputReducerBuffer(HadoopOpenCLContext clContext, Integer id) {
        super(clContext, id);
    ...
    public static class KeyValueIterator extends HadoopCLKeyValueIterator {
        protected IntFsvecHadoopCLOutputReducerBuffer[] buffers;
        private final IntFsvecHadoopCLOutputReducerBuffer buf;

        public KeyValueIterator(Deque<OutputBufferSoFar> toWrite, final int numReduceTasks,
                final IntFsvecHadoopCLOutputReducerBuffer buf) {
            this.buf = buf;
            this.buffers = new IntFsvecHadoopCLOutputReducerBuffer[toWrite.size()];
            int count = 0;
            for (OutputBufferSoFar tmp : toWrite) {
                this.buffers[count++] = (IntFsvecHadoopCLOutputReducerBuffer)tmp.buffer();
            }
            this.sortedIndices = new TreeSet<IntegerPair>(new Comparator<IntegerPair>() {
                @Override
                public int compare(IntegerPair a, IntegerPair b) {
                    final IntFsvecHadoopCLOutputReducerBuffer aBuf = buffers[a.buffer];
                    final IntFsvecHadoopCLOutputReducerBuffer bBuf = buffers[b.buffer];
        ...
        public ByteBuffer getKeyFor(IntegerPair index) throws IOException {
            final IntFsvecHadoopCLOutputReducerBuffer buffer = buffers[index.buffer];
