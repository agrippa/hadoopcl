...
public final class IntFloatHadoopCLOutputReducerBuffer extends HadoopCLOutputReducerBuffer implements KVCollection<IntWritable, FloatWritable> {
    ...
    public final Writable getValueFor(int index, Writable genericRef) {
        final FloatWritable ref = (FloatWritable)genericRef;
        final FloatWritable out;
        if (ref != null) {
            ref.set(this.outputVals[index]);
            out = ref;
        } else {
            out = new FloatWritable(this.outputVals[index]);
        }
        return out;
    }
    @Override
    ...
    public final void serializeValue(int index, DataOutputStream out) throws IOException {
        out.writeFloat(this.outputVals[index]);
    }
    ...
    @Override
    public Class<?> getOutputValClass() { return FloatWritable.class; }

    public IntFloatHadoopCLOutputReducerBuffer(HadoopOpenCLContext clContext, Integer id) {
        super(clContext, id);

            outputKeys = new int[this.clContext.getOutputBufferSize()];
            outputVals = new float[this.clContext.getOutputBufferSize()];
    }
    ...
    public static class KeyValueIterator extends HadoopCLKeyValueIterator {
        protected IntFloatHadoopCLOutputReducerBuffer[] buffers;
        private final IntFloatHadoopCLOutputReducerBuffer buf;

        public KeyValueIterator(Deque<OutputBufferSoFar> toWrite, final int numReduceTasks,
                final IntFloatHadoopCLOutputReducerBuffer buf) {
            this.buf = buf;
            this.buffers = new IntFloatHadoopCLOutputReducerBuffer[toWrite.size()];
            int count = 0;
            for (OutputBufferSoFar tmp : toWrite) {
                this.buffers[count++] = (IntFloatHadoopCLOutputReducerBuffer)tmp.buffer();
            }
            this.sortedIndices = new TreeSet<IntegerPair>(new Comparator<IntegerPair>() {
                @Override
                public int compare(IntegerPair a, IntegerPair b) {
                    final IntFloatHadoopCLOutputReducerBuffer aBuf = buffers[a.buffer];
                    final IntFloatHadoopCLOutputReducerBuffer bBuf = buffers[b.buffer];
        ...
        public ByteBuffer getKeyFor(IntegerPair index) throws IOException {
            final IntFloatHadoopCLOutputReducerBuffer buffer = buffers[index.buffer];
       ...
       public ByteBuffer getValueFor(IntegerPair index) throws IOException {
            final IntFloatHadoopCLOutputReducerBuffer buffer = buffers[index.buffer];
            this.valueBytes = resizeByteBuffer(this.valueBytes, 4);
