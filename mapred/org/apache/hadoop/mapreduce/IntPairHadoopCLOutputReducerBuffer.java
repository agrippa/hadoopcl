...
public final class IntPairHadoopCLOutputReducerBuffer extends HadoopCLOutputReducerBuffer implements KVCollection<IntWritable, PairWritable> {
    ...
    public IntPairHadoopCLOutputReducerBuffer(HadoopOpenCLContext clContext, Integer id) {
        super(clContext, id);
    ...
    public static class KeyValueIterator extends HadoopCLKeyValueIterator {
        protected IntPairHadoopCLOutputReducerBuffer[] buffers;
        private final IntPairHadoopCLOutputReducerBuffer buf;

        public KeyValueIterator(Deque<OutputBufferSoFar> toWrite, final int numReduceTasks,
                final IntPairHadoopCLOutputReducerBuffer buf) {
            this.buf = buf;
            this.buffers = new IntPairHadoopCLOutputReducerBuffer[toWrite.size()];
            int count = 0;
            for (OutputBufferSoFar tmp : toWrite) {
                this.buffers[count++] = (IntPairHadoopCLOutputReducerBuffer)tmp.buffer();
            }
            this.sortedIndices = new TreeSet<IntegerPair>(new Comparator<IntegerPair>() {
                @Override
                public int compare(IntegerPair a, IntegerPair b) {
                    final IntPairHadoopCLOutputReducerBuffer aBuf = buffers[a.buffer];
                    final IntPairHadoopCLOutputReducerBuffer bBuf = buffers[b.buffer];
        ...
        public ByteBuffer getKeyFor(IntegerPair index) throws IOException {
            final IntPairHadoopCLOutputReducerBuffer buffer = buffers[index.buffer];
