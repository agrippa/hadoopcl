...
public final class IntBsvecHadoopCLOutputMapperBuffer extends HadoopCLOutputMapperBuffer implements KVCollection<IntWritable, BSparseVectorWritable> {
    ...
    public IntBsvecHadoopCLOutputMapperBuffer(HadoopOpenCLContext clContext, Integer id) {
        super(clContext, id);
    ...
    public static class KeyValueIterator extends HadoopCLKeyValueIterator {
        protected IntBsvecHadoopCLOutputMapperBuffer[] buffers;
        private final IntBsvecHadoopCLOutputMapperBuffer buf;

        public KeyValueIterator(Deque<OutputBufferSoFar> toWrite, final int numReduceTasks,
                final IntBsvecHadoopCLOutputMapperBuffer buf) {
            this.buf = buf;
            this.buffers = new IntBsvecHadoopCLOutputMapperBuffer[toWrite.size()];
            int count = 0;
            for (OutputBufferSoFar tmp : toWrite) {
                this.buffers[count++] = (IntBsvecHadoopCLOutputMapperBuffer)tmp.buffer();
            }
            this.sortedIndices = new TreeSet<IntegerPair>(new Comparator<IntegerPair>() {
                @Override
                public int compare(IntegerPair a, IntegerPair b) {
                    final IntBsvecHadoopCLOutputMapperBuffer aBuf = buffers[a.buffer];
                    final IntBsvecHadoopCLOutputMapperBuffer bBuf = buffers[b.buffer];
        ...
        public ByteBuffer getKeyFor(IntegerPair index) throws IOException {
            final IntBsvecHadoopCLOutputMapperBuffer buffer = buffers[index.buffer];
