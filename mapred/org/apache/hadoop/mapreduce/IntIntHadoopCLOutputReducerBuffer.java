...
public final class IntIntHadoopCLOutputReducerBuffer extends HadoopCLOutputReducerBuffer implements KVCollection<IntWritable, IntWritable> {
    ...
    public final Writable getValueFor(int index, Writable genericRef) {
        ...
        if (ref != null) {
            ...
            out = new IntWritable(this.outputVals[index]);
        }
        return out;
    }
    @Override
    ...
    public final void serializeValue(int index, DataOutputStream out) throws IOException {
        out.writeInt(this.outputVals[index]);
    }
    ...
    @Override
    public Class<?> getOutputValClass() { return IntWritable.class; }

    public IntIntHadoopCLOutputReducerBuffer(HadoopOpenCLContext clContext, Integer id) {
        super(clContext, id);

            outputKeys = new int[this.clContext.getOutputBufferSize()];
            outputVals = new int[this.clContext.getOutputBufferSize()];
    }
    ...
    public static class KeyValueIterator extends HadoopCLKeyValueIterator {
        protected IntIntHadoopCLOutputReducerBuffer[] buffers;
        private final IntIntHadoopCLOutputReducerBuffer buf;

        public KeyValueIterator(Deque<OutputBufferSoFar> toWrite, final int numReduceTasks,
                final IntIntHadoopCLOutputReducerBuffer buf) {
            this.buf = buf;
            this.buffers = new IntIntHadoopCLOutputReducerBuffer[toWrite.size()];
            int count = 0;
            for (OutputBufferSoFar tmp : toWrite) {
                this.buffers[count++] = (IntIntHadoopCLOutputReducerBuffer)tmp.buffer();
            }
            this.sortedIndices = new TreeSet<IntegerPair>(new Comparator<IntegerPair>() {
                @Override
                public int compare(IntegerPair a, IntegerPair b) {
                    final IntIntHadoopCLOutputReducerBuffer aBuf = buffers[a.buffer];
                    final IntIntHadoopCLOutputReducerBuffer bBuf = buffers[b.buffer];
        ...
        public ByteBuffer getKeyFor(IntegerPair index) throws IOException {
            final IntIntHadoopCLOutputReducerBuffer buffer = buffers[index.buffer];
