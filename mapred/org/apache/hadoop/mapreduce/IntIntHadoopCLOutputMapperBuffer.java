...
public final class IntIntHadoopCLOutputMapperBuffer extends HadoopCLOutputMapperBuffer implements KVCollection<IntWritable, IntWritable> {
    ...
    public IntIntHadoopCLOutputMapperBuffer(HadoopOpenCLContext clContext, Integer id) {
        super(clContext, id);
    ...
    public static class KeyValueIterator extends HadoopCLKeyValueIterator {
        protected IntIntHadoopCLOutputMapperBuffer[] buffers;
        private final IntIntHadoopCLOutputMapperBuffer buf;

        public KeyValueIterator(Deque<OutputBufferSoFar> toWrite, final int numReduceTasks,
                final IntIntHadoopCLOutputMapperBuffer buf) {
            this.buf = buf;
            this.buffers = new IntIntHadoopCLOutputMapperBuffer[toWrite.size()];
            int count = 0;
            for (OutputBufferSoFar tmp : toWrite) {
                this.buffers[count++] = (IntIntHadoopCLOutputMapperBuffer)tmp.buffer();
            }
            this.sortedIndices = new TreeSet<IntegerPair>(new Comparator<IntegerPair>() {
                @Override
                public int compare(IntegerPair a, IntegerPair b) {
                    final IntIntHadoopCLOutputMapperBuffer aBuf = buffers[a.buffer];
                    final IntIntHadoopCLOutputMapperBuffer bBuf = buffers[b.buffer];
        ...
        public ByteBuffer getKeyFor(IntegerPair index) throws IOException {
            final IntIntHadoopCLOutputMapperBuffer buffer = buffers[index.buffer];
