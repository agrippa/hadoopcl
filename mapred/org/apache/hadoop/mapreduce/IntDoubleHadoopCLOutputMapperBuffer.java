...
public final class IntDoubleHadoopCLOutputMapperBuffer extends HadoopCLOutputMapperBuffer implements KVCollection<IntWritable, DoubleWritable> {
    ...
    public IntDoubleHadoopCLOutputMapperBuffer(HadoopOpenCLContext clContext, Integer id) {
        super(clContext, id);
    ...
    public static class KeyValueIterator extends HadoopCLKeyValueIterator {
        protected IntDoubleHadoopCLOutputMapperBuffer[] buffers;
        private final IntDoubleHadoopCLOutputMapperBuffer buf;

        public KeyValueIterator(Deque<OutputBufferSoFar> toWrite, final int numReduceTasks,
                final IntDoubleHadoopCLOutputMapperBuffer buf) {
            this.buf = buf;
            this.buffers = new IntDoubleHadoopCLOutputMapperBuffer[toWrite.size()];
            int count = 0;
            for (OutputBufferSoFar tmp : toWrite) {
                this.buffers[count++] = (IntDoubleHadoopCLOutputMapperBuffer)tmp.buffer();
            }
            this.sortedIndices = new TreeSet<IntegerPair>(new Comparator<IntegerPair>() {
                @Override
                public int compare(IntegerPair a, IntegerPair b) {
                    final IntDoubleHadoopCLOutputMapperBuffer aBuf = buffers[a.buffer];
                    final IntDoubleHadoopCLOutputMapperBuffer bBuf = buffers[b.buffer];
        ...
        public ByteBuffer getKeyFor(IntegerPair index) throws IOException {
            final IntDoubleHadoopCLOutputMapperBuffer buffer = buffers[index.buffer];
