...
public final class IntSvecHadoopCLOutputMapperBuffer extends HadoopCLOutputMapperBuffer implements KVCollection<IntWritable, SparseVectorWritable> {
    ...
    public IntSvecHadoopCLOutputMapperBuffer(HadoopOpenCLContext clContext, Integer id) {
        super(clContext, id);
    ...
    public static class KeyValueIterator extends HadoopCLKeyValueIterator {
        protected IntSvecHadoopCLOutputMapperBuffer[] buffers;
        private final IntSvecHadoopCLOutputMapperBuffer buf;

        public KeyValueIterator(Deque<OutputBufferSoFar> toWrite, final int numReduceTasks,
                final IntSvecHadoopCLOutputMapperBuffer buf) {
            this.buf = buf;
            this.buffers = new IntSvecHadoopCLOutputMapperBuffer[toWrite.size()];
            int count = 0;
            for (OutputBufferSoFar tmp : toWrite) {
                this.buffers[count++] = (IntSvecHadoopCLOutputMapperBuffer)tmp.buffer();
            }
            this.sortedIndices = new TreeSet<IntegerPair>(new Comparator<IntegerPair>() {
                @Override
                public int compare(IntegerPair a, IntegerPair b) {
                    final IntSvecHadoopCLOutputMapperBuffer aBuf = buffers[a.buffer];
                    final IntSvecHadoopCLOutputMapperBuffer bBuf = buffers[b.buffer];
        ...
        public ByteBuffer getKeyFor(IntegerPair index) throws IOException {
            final IntSvecHadoopCLOutputMapperBuffer buffer = buffers[index.buffer];
