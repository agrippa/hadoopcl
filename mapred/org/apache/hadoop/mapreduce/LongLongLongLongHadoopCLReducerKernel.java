...
public abstract class LongLongLongLongHadoopCLReducerKernel extends HadoopCLReducerKernel {
    ...
    public void setup(long[] setInputkeys, long[] setInputvals, long[] setOutputkeys, long[] setOutputvals, int[] setKeyIndex, int[] setNWrites, int setNKeys, int setNVals, int[] setMemIncr, int setOutputsPerInput) {
    ...
    public void fill(HadoopCLInputBuffer genericInputBuffer, HadoopCLOutputBuffer genericOutputBuffer) {
        ...
        if(this.outputsPerInput < 0 && (outputBuffer.outputKeys == null || outputBuffer.outputKeys.length < inputBuffer.nKeys * inputBuffer.maxInputValsPerInputKey)) {
            outputBuffer.outputKeys = new long[inputBuffer.nKeys * inputBuffer.maxInputValsPerInputKey];
    ...
    public HadoopCLAccumulatedProfile javaProcess(TaskInputOutputContext context) throws InterruptedException, IOException {
        ...
        while(ctx.nextKeyValue()) {
            ...
            this.javaProfile.startKernel();
            reduce(key.get()
, new HadoopCLLongValueIterator((long[])this.bufferedVals.getArray(), this.bufferedVals.size()));
            this.javaProfile.stopKernel();
