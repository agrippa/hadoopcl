...
public final class DoubleDoubleHadoopCLOutputReducerBuffer extends HadoopCLOutputReducerBuffer implements KVCollection<DoubleWritable, DoubleWritable> {
    ...
    public final Writable getKeyFor(int index, Writable genericRef) {
        ...
        if (ref != null) {
            ...
            out = new DoubleWritable(this.outputKeys[index]);
        }
        return out;
    }
    ...
    public final void serializeKey(int index, DataOutputStream out) throws IOException {
        out.writeDouble(this.outputKeys[index]);
    }
    @Override
    ...
    @Override
    public Class<?> getOutputKeyClass() { return DoubleWritable.class; }
    @Override
    public Class<?> getOutputValClass() { return DoubleWritable.class; }

    public DoubleDoubleHadoopCLOutputReducerBuffer(HadoopOpenCLContext clContext, Integer id) {
        super(clContext, id);

            outputKeys = new double[this.clContext.getOutputBufferSize()];
    ...
    public final int getPartitionFor(int index, int numReduceTasks) {
        return (((int)Double.doubleToLongBits(this.outputKeys[index])) & Integer.MAX_VALUE) % numReduceTasks;
    }


    public static class KeyValueIterator extends HadoopCLKeyValueIterator {
        protected DoubleDoubleHadoopCLOutputReducerBuffer[] buffers;
        private final DoubleDoubleHadoopCLOutputReducerBuffer buf;

        public KeyValueIterator(Deque<OutputBufferSoFar> toWrite, final int numReduceTasks,
                final DoubleDoubleHadoopCLOutputReducerBuffer buf) {
            this.buf = buf;
            this.buffers = new DoubleDoubleHadoopCLOutputReducerBuffer[toWrite.size()];
            int count = 0;
            for (OutputBufferSoFar tmp : toWrite) {
                this.buffers[count++] = (DoubleDoubleHadoopCLOutputReducerBuffer)tmp.buffer();
            }
            this.sortedIndices = new TreeSet<IntegerPair>(new Comparator<IntegerPair>() {
                @Override
                public int compare(IntegerPair a, IntegerPair b) {
                    final DoubleDoubleHadoopCLOutputReducerBuffer aBuf = buffers[a.buffer];
                    final DoubleDoubleHadoopCLOutputReducerBuffer bBuf = buffers[b.buffer];
        ...
        public ByteBuffer getKeyFor(IntegerPair index) throws IOException {
            final DoubleDoubleHadoopCLOutputReducerBuffer buffer = buffers[index.buffer];
