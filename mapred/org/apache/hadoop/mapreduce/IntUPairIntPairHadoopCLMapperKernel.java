...
public abstract class IntUPairIntPairHadoopCLMapperKernel extends HadoopCLMapperKernel {
    ...
    protected abstract void map(int key, int valId, double val1, double val2);

    public void setup(int[] setInputkeys, int[] setInputvalIds, double[] setInputvals1, double[] setInputvals2, int[] setOutputkeys, double[] setOutputvals1, double[] setOutputvals2, int[] setNWrites, int setNPairs, int[] setMemIncr, int setOutputsPerInput) {
    ...
    public Class<? extends HadoopCLInputBuffer> getInputBufferClass() { return IntUPairHadoopCLInputMapperBuffer.class; }
    ...
    public void fill(HadoopCLInputBuffer genericInputBuffer, HadoopCLOutputBuffer genericOutputBuffer) {
        IntUPairHadoopCLInputMapperBuffer inputBuffer = (IntUPairHadoopCLInputMapperBuffer)genericInputBuffer;
        IntPairHadoopCLOutputMapperBuffer outputBuffer = (IntPairHadoopCLOutputMapperBuffer)genericOutputBuffer;
        this.setup(inputBuffer.inputKeys, inputBuffer.inputValIds, inputBuffer.inputVals1, inputBuffer.inputVals2, outputBuffer.outputKeys, outputBuffer.outputVals1, outputBuffer.outputVals2, inputBuffer.nWrites, inputBuffer.nPairs, outputBuffer.memIncr, this.outputsPerInput);
    }
    ...
    protected void callMap() {
        map(inputKeys[3], inputValIds[3], inputVals1[3], inputVals2[3]);
    }
    @Override
    public HadoopCLAccumulatedProfile javaProcess(TaskInputOutputContext context) throws InterruptedException, IOException {
        ...
        while(ctx.nextKeyValue()) {
            ...
            UniquePairWritable val = (UniquePairWritable)ctx.getCurrentValue();
            this.javaProfile.stopRead();
            this.javaProfile.startKernel();
            map(key.get()
, val.getIVal(), val.getVal1(), val.getVal2()
);
            this.javaProfile.stopKernel();
