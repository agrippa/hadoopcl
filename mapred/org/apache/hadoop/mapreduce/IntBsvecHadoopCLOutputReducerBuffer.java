...
public final class IntBsvecHadoopCLOutputReducerBuffer extends HadoopCLOutputReducerBuffer implements KVCollection<IntWritable, BSparseVectorWritable> {
    ...
    public final Writable getValueFor(int index, Writable genericRef) {
        final BSparseVectorWritable ref = (BSparseVectorWritable)genericRef;
        final BSparseVectorWritable out;
        if (ref != null) {
    ...
    @Override
    public Class<?> getOutputValClass() { return BSparseVectorWritable.class; }

    public IntBsvecHadoopCLOutputReducerBuffer(HadoopOpenCLContext clContext, Integer id) {
        super(clContext, id);
    ...
    public static class KeyValueIterator extends HadoopCLKeyValueIterator {
        protected IntBsvecHadoopCLOutputReducerBuffer[] buffers;
        private final IntBsvecHadoopCLOutputReducerBuffer buf;

        public KeyValueIterator(Deque<OutputBufferSoFar> toWrite, final int numReduceTasks,
                final IntBsvecHadoopCLOutputReducerBuffer buf) {
            this.buf = buf;
            this.buffers = new IntBsvecHadoopCLOutputReducerBuffer[toWrite.size()];
            int count = 0;
            for (OutputBufferSoFar tmp : toWrite) {
                this.buffers[count++] = (IntBsvecHadoopCLOutputReducerBuffer)tmp.buffer();
            }
            this.sortedIndices = new TreeSet<IntegerPair>(new Comparator<IntegerPair>() {
                @Override
                public int compare(IntegerPair a, IntegerPair b) {
                    final IntBsvecHadoopCLOutputReducerBuffer aBuf = buffers[a.buffer];
                    final IntBsvecHadoopCLOutputReducerBuffer bBuf = buffers[b.buffer];
        ...
        public ByteBuffer getKeyFor(IntegerPair index) throws IOException {
            final IntBsvecHadoopCLOutputReducerBuffer buffer = buffers[index.buffer];
