...
public abstract class IntIvecIntIntHadoopCLMapperKernel extends HadoopCLMapperKernel {
    ...
    public void setup(int[] setInputkeys, int[] setInputvalLookAsideBuffer, int[] setInputval, int[] setOutputkeys, int[] setOutputvals, int[] setNWrites, int setNPairs, int setIndividualInputValsCount, int[] setMemIncr, int setOutputsPerInput) {
    ...
    public void fill(HadoopCLInputBuffer genericInputBuffer, HadoopCLOutputBuffer genericOutputBuffer) {
        ...
        if (inputBuffer.enableStriding) {
            ...
            while (lengthIter.hasNext()) {
                ...
                while (pairsIter.hasNext()) {
                    ...
                    inputBuffer.inputVal.ensureCapacity( (index + ((curr.length - 1) * inputBuffer.nVectorsToBuffer)) + 1);
                    for (int i = 0; i < curr.length; i++) {
                        inputBuffer.inputVal.unsafeSet(index + (i * inputBuffer.nVectorsToBuffer), curr.indices[i]);
                    }
        ...
        this.setup(inputBuffer.inputKeys, inputBuffer.inputValLookAsideBuffer, (int[])(inputBuffer.inputVal.getArray()), outputBuffer.outputKeys, outputBuffer.outputVals, inputBuffer.nWrites, inputBuffer.nPairs, inputBuffer.individualInputValsCount, outputBuffer.memIncr, this.outputsPerInput);
    }
