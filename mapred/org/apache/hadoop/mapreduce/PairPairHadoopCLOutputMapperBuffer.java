...
public final class PairPairHadoopCLOutputMapperBuffer extends HadoopCLOutputMapperBuffer implements KVCollection<PairWritable, PairWritable> {
    ...
    public final Writable getKeyFor(int index, Writable genericRef) {
        final PairWritable ref = (PairWritable)genericRef;
        final PairWritable out;
        if (ref != null) {
    ...
    public final void serializeKey(int index, DataOutputStream out) throws IOException {
        out.writeDouble(this.outputKeys1[index]);
        out.writeDouble(this.outputKeys2[index]);
    }
    @Override
    ...
    @Override
    public Class<?> getOutputKeyClass() { return PairWritable.class; }
    @Override
    public Class<?> getOutputValClass() { return PairWritable.class; }

    public PairPairHadoopCLOutputMapperBuffer(HadoopOpenCLContext clContext, Integer id) {
        super(clContext, id);

            outputKeys1 = new double[this.clContext.getOutputBufferSize()];
            outputKeys2 = new double[this.clContext.getOutputBufferSize()];
    ...
    public long space() {
        return super.space() + 
            (outputKeys1.length * 8) +
            (outputKeys2.length * 8) +
    ...
    public final int getPartitionFor(int index, int numReduceTasks) {
        return (((int)this.outputKeys1[index] + (int)this.outputKeys2[index]) & Integer.MAX_VALUE) % numReduceTasks;
    }


    public static class KeyValueIterator extends HadoopCLKeyValueIterator {
        protected PairPairHadoopCLOutputMapperBuffer[] buffers;
        private final PairPairHadoopCLOutputMapperBuffer buf;

        public KeyValueIterator(Deque<OutputBufferSoFar> toWrite, final int numReduceTasks,
                final PairPairHadoopCLOutputMapperBuffer buf) {
            this.buf = buf;
            this.buffers = new PairPairHadoopCLOutputMapperBuffer[toWrite.size()];
            int count = 0;
            for (OutputBufferSoFar tmp : toWrite) {
                this.buffers[count++] = (PairPairHadoopCLOutputMapperBuffer)tmp.buffer();
            }
            this.sortedIndices = new TreeSet<IntegerPair>(new Comparator<IntegerPair>() {
                @Override
                public int compare(IntegerPair a, IntegerPair b) {
                    final PairPairHadoopCLOutputMapperBuffer aBuf = buffers[a.buffer];
                    final PairPairHadoopCLOutputMapperBuffer bBuf = buffers[b.buffer];
        ...
        public ByteBuffer getKeyFor(IntegerPair index) throws IOException {
            final PairPairHadoopCLOutputMapperBuffer buffer = buffers[index.buffer];
            this.keyBytes = resizeByteBuffer(this.keyBytes, 16);
       ...
       public ByteBuffer getValueFor(IntegerPair index) throws IOException {
            final PairPairHadoopCLOutputMapperBuffer buffer = buffers[index.buffer];
            this.valueBytes = resizeByteBuffer(this.valueBytes, 16);
