...
public final class DoubleDoubleHadoopCLInputMapperBuffer extends HadoopCLInputMapperBuffer {
    public  double[] inputKeys;
    public  double[] inputVals;


    public DoubleDoubleHadoopCLInputMapperBuffer(HadoopOpenCLContext clContext, Integer id) {
        super(clContext, id);

        inputKeys = new double[this.clContext.getInputBufferSize()];
    ...
    public final int bulkFill(HadoopCLDataInput stream) throws IOException {
        ...
        while (stream.hasMore() &&
                ...
                true) {
            stream.nextKey();
            final double tmpKey = stream.readDouble();
            stream.nextValue();
