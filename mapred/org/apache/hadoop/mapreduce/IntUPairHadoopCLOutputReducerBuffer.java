...
public final class IntUPairHadoopCLOutputReducerBuffer extends HadoopCLOutputReducerBuffer implements KVCollection<IntWritable, UniquePairWritable> {
    ...
    public IntUPairHadoopCLOutputReducerBuffer(HadoopOpenCLContext clContext, Integer id) {
        super(clContext, id);
    ...
    public static class KeyValueIterator extends HadoopCLKeyValueIterator {
        protected IntUPairHadoopCLOutputReducerBuffer[] buffers;
        private final IntUPairHadoopCLOutputReducerBuffer buf;

        public KeyValueIterator(Deque<OutputBufferSoFar> toWrite, final int numReduceTasks,
                final IntUPairHadoopCLOutputReducerBuffer buf) {
            this.buf = buf;
            this.buffers = new IntUPairHadoopCLOutputReducerBuffer[toWrite.size()];
            int count = 0;
            for (OutputBufferSoFar tmp : toWrite) {
                this.buffers[count++] = (IntUPairHadoopCLOutputReducerBuffer)tmp.buffer();
            }
            this.sortedIndices = new TreeSet<IntegerPair>(new Comparator<IntegerPair>() {
                @Override
                public int compare(IntegerPair a, IntegerPair b) {
                    final IntUPairHadoopCLOutputReducerBuffer aBuf = buffers[a.buffer];
                    final IntUPairHadoopCLOutputReducerBuffer bBuf = buffers[b.buffer];
        ...
        public ByteBuffer getKeyFor(IntegerPair index) throws IOException {
            final IntUPairHadoopCLOutputReducerBuffer buffer = buffers[index.buffer];
