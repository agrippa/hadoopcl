...
public class HadoopCLTaskCharacterization2 extends AHadoopCLTaskCharacterization<Integer, Double> {

    public HadoopCLTaskCharacterization2(String taskName, int setTotalDevices, boolean isMapper) {
    ...
    public int predictBestDevice(int[] occupancy, HadoopCLDeviceChecker checker)
        ...
        for(int i = 0; i < this.totalNumDevices; i++) {
            if(checker.validDeviceForTask(i)) {
                double prediction = predict(i, (double)occupancy[i]);
