...
public abstract class IntFsvecIntIntHadoopCLMapperKernel extends HadoopCLMapperKernel {
    ...
    public void setup(int[] setInputkeys, int[] setInputvalLookAsideBuffer, int[] setInputvalIndices, float[] setInputvalVals, int[] setOutputkeys, int[] setOutputvals, int[] setNWrites, int setNPairs, int setIndividualInputValsCount, int[] setMemIncr, int setOutputsPerInput) {
    ...
    public void fill(HadoopCLInputBuffer genericInputBuffer, HadoopCLOutputBuffer genericOutputBuffer) {
        ...
        if (inputBuffer.enableStriding) {
            ...
            while (lengthIter.hasNext()) {
                ...
                while (pairsIter.hasNext()) {
                    ...
                    inputBuffer.inputValIndices.ensureCapacity( (index + ((curr.length - 1) * inputBuffer.nPairs)) + 1);
                    inputBuffer.inputValVals.ensureCapacity( (index + ((curr.length - 1) * inputBuffer.nPairs)) + 1);
                    for (int i = 0; i < curr.length; i++) {
                        inputBuffer.inputValIndices.unsafeSet(index + (i * inputBuffer.nPairs), curr.indices[i]);
                        inputBuffer.inputValVals.unsafeSet(index + (i * inputBuffer.nPairs), curr.fvals[i]);
                    }
        ...
        this.setup(inputBuffer.inputKeys, inputBuffer.inputValLookAsideBuffer, (int[])(inputBuffer.inputValIndices.getArray()), (float[])(inputBuffer.inputValVals.getArray()), outputBuffer.outputKeys, outputBuffer.outputVals, inputBuffer.nWrites, inputBuffer.nPairs, inputBuffer.individualInputValsCount, outputBuffer.memIncr, this.outputsPerInput);
    }
