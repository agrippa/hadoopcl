...
public final class LongLongHadoopCLOutputReducerBuffer extends HadoopCLOutputReducerBuffer implements KVCollection<LongWritable, LongWritable> {
    ...
    public final Writable getKeyFor(int index, Writable genericRef) {
        final LongWritable ref = (LongWritable)genericRef;
        final LongWritable out;
        if (ref != null) {
            ref.set(this.outputKeys[index]);
            out = ref;
        } else {
            out = new LongWritable(this.outputKeys[index]);
        }
        return out;
    }
    ...
    @Override
    public Class<?> getOutputKeyClass() { return LongWritable.class; }
    @Override
    public Class<?> getOutputValClass() { return LongWritable.class; }

    public LongLongHadoopCLOutputReducerBuffer(HadoopOpenCLContext clContext, Integer id) {
        super(clContext, id);

            outputKeys = new long[this.clContext.getOutputBufferSize()];
            outputVals = new long[this.clContext.getOutputBufferSize()];
    }
    ...
    public final int getPartitionFor(int index, int numReduceTasks) {
        return (((int)this.outputKeys[index]) & Integer.MAX_VALUE) % numReduceTasks;
    }


    public static class KeyValueIterator extends HadoopCLKeyValueIterator {
        protected LongLongHadoopCLOutputReducerBuffer[] buffers;
        private final LongLongHadoopCLOutputReducerBuffer buf;

        public KeyValueIterator(Deque<OutputBufferSoFar> toWrite, final int numReduceTasks,
                final LongLongHadoopCLOutputReducerBuffer buf) {
            this.buf = buf;
            this.buffers = new LongLongHadoopCLOutputReducerBuffer[toWrite.size()];
            int count = 0;
            for (OutputBufferSoFar tmp : toWrite) {
                this.buffers[count++] = (LongLongHadoopCLOutputReducerBuffer)tmp.buffer();
            }
            this.sortedIndices = new TreeSet<IntegerPair>(new Comparator<IntegerPair>() {
                @Override
                public int compare(IntegerPair a, IntegerPair b) {
                    final LongLongHadoopCLOutputReducerBuffer aBuf = buffers[a.buffer];
                    final LongLongHadoopCLOutputReducerBuffer bBuf = buffers[b.buffer];
        ...
        public ByteBuffer getKeyFor(IntegerPair index) throws IOException {
            final LongLongHadoopCLOutputReducerBuffer buffer = buffers[index.buffer];
            this.keyBytes = resizeByteBuffer(this.keyBytes, 8);
