...
class IvecVisitor(NativeTypeVisitor):
    ...
    def getAddValueMethodMapper(self):
        return [ 'this.inputValLookAsideBuffer[this.nPairs] = this.individualInputValsCount;',
                 ...
                 # '    }',
                 '} else {',
                 '    System.arraycopy(actual.vals(), actual.valsOffset(), this.inputVal, this.individualInputValsCount, actual.size());',
                 # '    for (int i = 0; i < actual.size(); i++) {',
...
def writeKeyValueIteratorDefs(fp, nativeOutputKeyType, nativeOutputValueType, isMapper):
    ...
    fp.write('\n')
    fp.write('        public KeyValueIterator(Deque<OutputBufferSoFar> toWrite, final int numReduceTasks,\n')
    ...
    fp.write('\n')
    fp.write('    @Override\n')
    fp.write('    public HadoopCLKeyValueIterator getKeyValueIterator(Deque<OutputBufferSoFar> toWrite, int numReduceTasks) {\n')
